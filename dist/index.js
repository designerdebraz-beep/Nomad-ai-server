...
const seedDatabase = async () => {
    try {
        if ((0, db_1.isJsonDbActive)()) {
            ...
            const count = await Destination_1.DestinationModel.countDocuments();
            if (count === 0) {
                console.log('Seeding MongoDB Database...');
                await Destination_1.DestinationModel.insertMany(seedDestinations);
...
app.get('/api/health', (req, res) => {
    res.json({
        status: 'ok',
        database: (0, db_1.isJsonDbActive)() ? 'local_json_db' : 'mongodb',
        timestamp: new Date()
    });
});
// Start Server
