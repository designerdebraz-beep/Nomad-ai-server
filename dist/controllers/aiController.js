...
exports.chatAssistant = exports.getRecommendations = void 0;
const db_1 = require("../config/db");
const Destination_1 = require("../models/Destination");
// Helper to get all destinations for AI context
const getAllDestinationsContext = async () => {
    if ((0, db_1.isJsonDbActive)()) {
        return (0, db_1.readJsonDb)().destinations;
    }
    else {
        return await Destination_1.DestinationModel.find({});
    }
};
...
const getMockRecommendations = (pref, destinations) => {
    ...
    const scoredMatches = matches.map((d) => {
        ...
        // Climate matches
        if (climate === 'Warm' && d.weatherTemp >= 24)
            score += 15;
        else if (climate === 'Cool' && d.weatherTemp <= 18)
            score += 15;
        else if (climate === 'Mild' && d.weatherTemp > 18 && d.weatherTemp < 24)
            score += 15;
        // Interest matches (mock mapping)
        const desc = (d.shortDescription + ' ' + d.fullDescription).toLowerCase();
        if (interest === 'Beaches' && (desc.includes('beach') || desc.includes('coast') || desc.includes('ocean')))
            score += 10;
        if (interest === 'Nightlife' && (desc.includes('nightlife') || desc.includes('club') || desc.includes('bars')))
            score += 10;
        if (interest === 'Culture' && (desc.includes('culture') || desc.includes('history') || desc.includes('temple')))
            score += 10;
        if (interest === 'Nature' && (desc.includes('nature') || desc.includes('mountains') || desc.includes('forest')))
            score += 10;
        if (interest === 'Tech Hub' && (desc.includes('tech') || desc.includes('startup') || desc.includes('digital')))
            score += 10;
...
const getMockChatResponse = (message, history, activeCity) => {
...
const getRecommendations = async (req, res) => {
    try {
      ...
      `;
            ...
            const enriched = jsonRes.map((rec) => {
